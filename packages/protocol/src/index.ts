 ...
 *
 * Accepts an ArrayBuffer (what a browser WebSocket hands over) or a
 * Uint8Array/Buffer (what the Node server has). Returns null for anything too
 * short to carry a header, rather than a frame with garbage in it.
 */
export function decode(data: ArrayBuffer | Uint8Array): DecodedFrame | null {
  ...
  if (cmd === CMD.DATA && bytes.length >= DATA_HEADER_BYTES) {
